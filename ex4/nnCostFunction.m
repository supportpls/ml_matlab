...
J=(sum(sum(-y_k.*log(a3)))-sum(sum((1-y_k).*log(1-a3))))/m+...
  ((lambda/(2*m))*((sum(sum(Theta1_reg)))+sum(sum(Theta2_reg))));
...
Theta2_grad_reg(:,1) = 0; 
Theta1_grad = delta1/m+lambda/m*Theta1_grad_reg;
Theta2_grad = delta2/m+lambda/m*Theta2_grad_reg;
